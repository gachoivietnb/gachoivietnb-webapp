...
export default async function PayablesStatementPage() {
  ...
  const items: StmtItem[] = purchases.map((p) => {
    ...
    return {
      ...
      partner_id: s?.id ?? null,
      partner_name: s?.name ?? null,
      kind: p.kind ?? 'ga',
    }
  })

  const payments: StmtPayment[] = (
    (paysRes.data ?? []) as Array<{ purchase_id: string; payment_date: string; amount: number }>
  ).map((p) => ({ item_id: p.purchase_id, date: p.payment_date, amount: Number(p.amount) }))
  ...
  return (
    <div>
      <div className="mb-5">
        ...
        <h1 className="text-2xl font-medium text-gray-900 dark:text-gray-100 flex items-center gap-2">📊 Báo cáo công nợ phải trả</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Số dư đầu kỳ · phát sinh tăng/giảm · số dư cuối kỳ · lọc theo kỳ, NCC & loại
        </p>
      </div>
      <DebtStatementClient side="payable" items={items} payments={payments} partners={partners} />
    </div>
  )
}
