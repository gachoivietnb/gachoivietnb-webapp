...
export function PurchaseReceiptActions({
  ...
  async function download(format: 'excel' | 'pdf') {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/purchases/${purchaseId}/export?format=${format}`)
      if (!res.ok) {
      ...
      a.href = href
      a.download = `phieu-nhap_${purchaseCode}.${format === 'pdf' ? 'pdf' : 'xlsx'}`
